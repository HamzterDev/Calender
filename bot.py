...
# ======================
# LOAD TELEGRAM BOT TOKEN FROM TXT
# ======================
...
def load_bot_token():
    ...
    if not os.path.isfile(token_file):
        raise RuntimeError("❌ ไม่พบไฟล์ bot_token.txt")

    with open(token_file, "r", encoding="utf-8") as f:
        token = f.read().strip()

    if not token:
        raise RuntimeError("❌ BOT_TOKEN ว่าง")
...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" in text:
        ...
        add_event(text, datetime.now())

    await update.message.reply_text("✅ เพิ่มงานแล้ว")
